<?php
/*
 * Plugin Name: Razorpay Route for WooCommerce
 * Plugin URI: https://razorpay.com
 * Description: Razorpay Route plugin for WooCommerce
 * Version: 1
 * Author: Razorpay
...
require_once __DIR__.'/includes/rzp-route.php';
...
// print_r($data); exit();
            ...
            update_post_meta($orderId, 'rzp_order_id', $razorpayOrderId );
...
if($rzp_transfer_from == 'from_order'){
    ...
    for($i=0;$i<$LA_transfer_count;$i++){
        if(!empty($LA_number_arr[$i]) && !empty($LA_amount_arr[$i])){
                ...
                // print_r($order_transfer_arr); exit();
            $data = array(
                'receipt'         => $orderId,
...
EOT;
        ...
        public function updateOrder(& $order, $success, $errorMessage, $razorpayPaymentId, $virtualAccountId = null, $webhook = false)
        {
            ...
            if (($success === true) and ($order->needs_payment() === true))
            {
                ...
                $this->transferFromPayment($orderId, $razorpayPaymentId);
...
function product_transfer_data_fields()
{
    ...
    if(isset($LA_number_arr) && is_array($LA_number_arr) && isset($LA_amount_arr) && is_array($LA_amount_arr)) {
        ...
        for($i=0;$i<$LA_transfer_count;$i++){
            if(!empty($LA_number_arr[$i]) && !empty($LA_amount_arr[$i])){
                ...
                <input type="text" name="LA_transfer_amount[]" class="LA_transfer_amount" placeholder="Amount" value="'.$LA_amount_arr[$i].'">
                <label class="trf_settlement_label">Hold Settlement:</label>  <select name="LA_transfer_status[]"><optgroup label="On Hold">';
                echo '<option value="1"'; if($LA_trf_status_arr[$i] == 1){ echo "selected"; } echo '> Yes</option><option value="0"';if($LA_trf_status_arr[$i] == 0){ echo "selected"; } echo ' > No</option></optgroup></select> </p>';
             }
    ...
    echo '<p class="input_fields_wrap"> <a class="add_field_button button-secondary">Add Field</a>
        ...
        <input type="text" name="LA_transfer_amount[]"  class="LA_transfer_amount" placeholder="Amount" >
        <label class="trf_settlement_label">Hold Settlement:</label>
        <select name="LA_transfer_status[]"><optgroup label="On Hold">
                <option value="1"> Yes</option><option value="0" selected> No</option></optgroup></select>
    </p>
    </div>
